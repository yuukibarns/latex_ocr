...
class Im2LatexDataset:
    ...
    def __init__(
        ...
        if images is not None and equations is not None:
            ...
            # Normalize indices in case of index counted from non-zero
            self_indices_min = min(self.indices)
            for i in range(len(self.indices)):
                self.indices[i] -= self_indices_min
...
if __name__ == "__main__":
    ...
    if args.images is None and args.equations is not None:
        ...
        raise ValueError("Tokenizer not Found, generate it first")
